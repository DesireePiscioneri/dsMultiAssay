...
#' @export
MultiAssayDS <- function(exp, df = NULL, df_col = NULL, colD = DataFrame(),
                         levels_col = NULL, level = NULL,
                         sampleMap = DataFrame(assay = factor(), primary = character(), colname = character()),
  ...
  if (!is.null(df)) {
    ...
    if (is.null(df_col)){
      ...
      if (!is.null(levels_col)){
        ...
        rownames(df)<-df[[df_col]]
        df[df_col] <- NULL
        df <- as.data.frame(t(x))
        exp <- c(exp, list(df=df))
      }}}
